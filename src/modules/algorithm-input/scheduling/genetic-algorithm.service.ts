  ...
  private calculateFitness(chromosome: Chromosome): number {
    ...
    for (const gene of chromosome) {
      ...
      students.forEach((studentId) => {
        ...
        if (this.problem.constraints.avoidInterLocationTravel) {
    ...
    if (this.problem.constraints.maxExamsPerStudentPerDay) {
      studentDailySchedule.forEach((v) => {
        if (v.length > this.problem.constraints.maxExamsPerStudentPerDay!) {
